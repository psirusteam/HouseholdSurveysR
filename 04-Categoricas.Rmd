...
survey_design <- survey_design %>%
  mutate(
    ...
    age_18 = case_when(Age < 18 ~ "< 18 years", 
                       TRUE     ~ ">= 18 years")
  )
```
...
survey_design %>%
  ...
  cascade( size = survey_total(vartype =c("se","ci")),
           .fill = "Total")
```
...
sex_poverty_contrast <- svyby(formula = ~poor, by = ~Sex,
...
```{r}
svycontrast(sex_poverty_contrast,
            list(sex_differenceerence = c(1, -1))) %>%
  data.frame()
```
...
region_unemployment_contrast <- svyby(formula = ~unemployed, by = ~Region,
...
```{r}
svycontrast(region_unemployment_contrast, list(
...
poverty_design <- survey_design %>%
       mutate(poor = factor(poor, levels = 0:1,
...
svycontrast(stat = sex_poverty_odds_contrast,
...
ggplot(data = male_zone_poverty_proportion,
  aes(x = Poverty, y = proportion,
...
ggplot(data = male_region_poverty_proportion,
...
bivariate_data <- bi_class(bivariate_shape,
  y = proportion, x = proportion_cv, dim = k,
  style = "fisher")
