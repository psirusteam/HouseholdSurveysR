...
sex_poverty_proportion <- survey_design %>%
  group_by(Sex) %>%
  summarise(proportion = survey_mean(poverty,
                                     na.rm = TRUE,
                                     vartype = c("se", "ci")))

sex_poverty_contrast <- svyby(formula = ~poverty, by = ~Sex,
...
region_unemployment_proportion <- survey_design %>%
  ...
  group_by(Region) %>%
  summarise(proportion = survey_mean(unemployment,
                                     na.rm = TRUE,
                                     vartype = c("se", "ci")))

region_unemployment_contrast <- svyby(formula = ~unemployment, by = ~Region,
...
```{r eval=FALSE}
sex_by_poverty_proportion %>%
  dplyr::select(poverty, Sex, proportion_low, proportion_upp)
```
...
Si ahora el objetivo es estimar la pobreza por region, se utiliza la variable numerica `poverty` dentro de un flujo `srvyr`, como se muestra en la Tabla \@ref(tab:tab-svyby-region-pobreza).
...
sex_poverty_odds_contrast <- svymean(x = ~interaction(Sex, poverty),
                                     design = survey_design,
