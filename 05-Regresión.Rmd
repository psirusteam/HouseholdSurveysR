...
diseno$variables %<>% mutate(stdresids = stdresids)
```
...
g1 <- ggplot(data = diseno$variables,
       aes(x = Expenditure, y = stdresids)) +
  geom_point() +
  geom_hline(yintercept = 0) + theme_cepal()

g2 <- ggplot(data = diseno$variables,
       aes(x = Zone, y = stdresids)) +
  geom_point() +
  geom_hline(yintercept = 0) + theme_cepal()

g3 <- ggplot(data = diseno$variables,
...
library(svydiags)
svyCooksD(fit_svy, doplot=TRUE) %>% head(10)
```
...
```{r}
resumen <- summary(fit_svy)$coefficients %>%
  as.data.frame() %>%
...
Varmu <- as.numeric(xobs %*% cov_beta %*% t(xobs))  
Varmu %>% head(10)
```
...
mu_IC <- data.frame(confint(predict(fit_svy, type = "link")))
colnames(mu_IC) <- c("Lim_Inf", "Lim_Sup")
```
