...
```{r}
cov <- svyvar(~Income + Expenditure, design = survey_design)
```
...
```{r, eval=TRUE}
res <- svyttest(Income ~ Sex, design = survey_design, level = 0.95)
```
...
df_res <- tibble(
  ...
  estimated_difference = res$estimate
)
...
urban_subset <- survey_design %>%
  filter(Zone == "Urban")
res <- svyttest(Income ~ Sex, design = urban_subset, level = 0.95)
```
...
```{r}
res <- svycontrast(stat = region_mean_contrast,
...
df_con <- tibble::tibble(
  contrast = names(res),
  estimate = as.numeric(res),
  variance = attr(res, "var"),
  standard_error = sqrt(attr(res, "var")))
...
```{r,contr1_var,eval=TRUE}
# Step 2: variance-covariance matrix
mat <- vcov(region_mean_contrast)
```
...
df_con <- tibble(
  contrast = names(res),
  estimate = as.numeric(res),
  variance = diag(attr(res, "var")),
  standard_error = sqrt(diag(attr(res, "var")))
)
...
```{r}
res <- svycontrast(stat = sex_mean_contrast,
...
```{r}
res <- svycontrast(stat = sex_ratio_contrast,
                   contrasts = list(
                          sex_difference = c(1, -1))
                   )
